...
export function getService(id: string): Result<_Service, string> {
    ...
    return match(serviceStorage.get(id), {
        ...
        None: () => Result.Err<_Service, string>(`a service with id=${id} not found`)
    });
}
...
export function getClientsRequest(id: string): Result<Vec<Request>, string> {
    ...
    return match(clientStorage.get(id), {
        ...
        None: () => Result.Err<Vec<Request>, string>(`a client with id=${id} not found`)
    });
}
 ...
 */
$update;
export function checkOutRequest(cid: string, rid: string): Result<Request, string> {
